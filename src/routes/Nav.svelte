    ...
    onMount(() => {
		...
		connectToMetamask = async function() {
            ...
            try {
                ...
                if (error.code === 4902) {
                    // U2U has not been added, we will add here
                    try {
            ...
            <div class="lg:flex hidden items-center gap-[10px] mr-[4px] font-semibold">      
                <div class="relative flex flex-col">
                    <div class=" font-md text-white px-4 rounded-md pb-[1px]">
                        <img src="/coredao.svg" class="h-[25px]" alt="U2U Network"/>
                    </div>
                </div>
            </div>
...
<!--Mobile Nav menu-->
{#if navMenu}
    <div class="absolute fixed font-semibold flex flex-col inset-0 bg-black text-white bg-opacity-80 z-20 mt-[50px] text-black" in:fade={{ duration: 200 }} out:fade={{ duration: 200 }}>
        <button class=" flex text-xl py-1 bg-black  flex justify-center "  on:click={() =>goto(`/collections`)}  in:fade={{ duration: 300 }} out:fade={{ duration: 300 }}>
            Collections
        </button>
        <button class=" flex text-xl py-1 bg-black flex justify-center " on:click={() =>goto(`/mint`)} in:fade={{ duration: 400 }} out:fade={{ duration: 400 }}>
            Launchpad
        </button>
