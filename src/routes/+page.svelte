    ...
    onMount(() => {
        ...
        (async function getMintFeaturedCollections() {
            ...
            if (response.ok) {
                ...
                if (responseBody.success && responseBody.payload.liveDrops) {
                    // Filter out violated collections
                    const collections = responseBody.payload.liveDrops.filter(collection => !collection.isViolated);
                    ...
                    const updateStatuses = () => {
                        ...
                        const updatedCollections = collections.map(collection => {
                            const whitelistLive = collection.whitelistStart && collection.whitelistEnd && 
                                                currentTime >= collection.whitelistStart && currentTime <= collection.whitelistEnd;
                            const publicLive = collection.publicStart && collection.publicEnd && 
                                            currentTime >= collection.publicStart && currentTime <= collection.publicEnd;
                            ...
                            if (whitelistLive || publicLive) {
                                ...
                                // Check if upcoming
                                const whitelistUpcoming = collection.whitelistStart && currentTime < collection.whitelistStart;
                                const publicUpcoming = collection.publicStart && currentTime < collection.publicStart;
...
<div class="flex text-black font-Oxanium bg-black w-full min-h-screen">
    <div class="flex flex-col w-full max-w-screen-2xl mx-auto pb-[60px]">
        <div class="flex flex-col">
                                            ...
                                            <div class="left-0 bottom-0 absolute self-end flex flex-col bg-gradient-black-fade shadow-xl bg-opacity-90 h-[280px] xl:min-w-[60%]  max-w-[100%] min-w-[100%] px-4 py-4 sm:gap-[10px] gap-0">
                                                ...
                                                <div class="flex flex-wrap text-white sm:text-lg text-sm w-full justify-start lg:gap-[40px] sm:gap-[20px] gap-[15px]">
                                                    <div class="flex flex-col">
                                                        ...
                                                        <span>
                                                            {bigIntToFloat(BigInt(topCollection.floorPrice)).toFixed(4)} U2U
                                                        </span>
                                                    </div>
                                                    <div class="flex flex-col">
                                                        <span class="text-lightGray">
                                                        ...
                                                        <span>
                                                            {bigIntToFloat(BigInt(topCollection.volume)).toFixed(4)} U2U
                                                        </span>
                                                    </div>
                            ...
                            {/each}
                                <div bind:this={scrollContainer} class="absolute bottom-[20px] left-[20px] flex xl:gap-[15px] lg:gap-10 gap-4 overflow-x-scroll custom-scrollbar mt-[10px] ">
                                    ...
                                    {#each topCollections as topCollection}    
                                        ...
                                        class="relative group parent flex flex-col gap-1 rounded-xl cursor-pointer relative bg-launchpadDarkLight shadow-xl min-h-[100px] max-h-[100px] hover:border-blue hover:border-2">
                                            <!--Card img-->
                            ...
                            <div class="flex flex-col gap-[10px] rounded-b-sm">
                                {#each firstFiveMainPageCollections as firstFiveMainPageCollection, index}
                                    ...
                                    <div class="flex justify-between items-center hover:text-yellow-500 py-[5px]">
                                        ...
                                        <!--Floor price-->
                                        <div class="lg:flex hidden flex-grow hidden items-center lg:w-[25%]">
                                            <img class="h-[25px]  inline-block mr-[5px]" src="/icons/U2Usm.png" alt="small coin icon"/>
                                            <!--Offer value-->
            ...
            <!--Mint header-->
            <h1 class="text-white text-2xl font-semibold mt-[60px] gap-[6px] mx-2">Launchpad</h1>
            <button 
                    class="w-fit md:text-xl px-1 py-1 my-2 mx-2
        ...
        border-amber-300 shadow-lg transform hover:scale-105 transition-all duration-200"
            ...
            <div class="relative flex justify-between mb-[20px] mt-2">
                ...
                <div bind:this={scrollContainer} class="flex xl:gap-[33px] lg:gap-10 gap-4 overflow-x-scroll custom-scrollbar">
                    {#if mintCollections.length === 0}
                            ...
                            {#each loadingMintCollections as item}    
                                <!--Cards-->
                                <button  
                                class="relative group parent flex flex-col gap-1 rounded-xl cursor-pointer relative bg-launchpadDarkLight shadow-lg  min-h-[170px] max-h-[170px] px-2 py-2 hover:border-greenHover hover:border">
                                    <!--Card img-->
                ...
                <!--See all button-->
                <button class="absolute bottom-0 right-0 mb-[-20px] text-base p-2 bg-button hover:bg-buttonHover shadow-xl text-white font-semibold rounded-md lg:w-[200px] md:w-[150px] sm:w-[100px] w-[80px]" on:click={()=>{goto(`./mint`)}} >
                    See all
                </button>
            </div>
        </div>
